...
export function FinishScreen({ snapshot, onReplay, onNewRace }: {
  ...
  return (
    <AccessibleDialog className="finish-screen" labelledBy="finish-title" onClose={dismiss}>
      <header><p className="finish-screen__eyebrow">Chequered flag · {SHANGHAI_RACE_LAPS} laps</p><h2 id="finish-title">Race complete</h2><p>{DRIVERS_2026.find((driver) => driver.id === winner?.driverId)?.name} wins in Shanghai in {formatDuration(winner?.timing.totalTime ?? 0)}</p></header>
